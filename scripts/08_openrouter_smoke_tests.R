# ==============================================================================
# 08_openrouter_condition_a_pilot.R
#
# PURPOSE
# Smoke-test the real OpenRouter data collection pipeline on the 3 cheapest
# models from your final 11-model list, before running the full study.
# Condition A only (verbatim GSS wording). Covers ALL FOUR issues (abortion,
# gender roles, police violence, suicide/euthanasia), each variable run across
# first a 3-year fixed subset and then every year the GSS actually fielded 
# that item.
#
# WHAT CHANGED FROM THE LAST VERSION AND WHY
#   1. Abortion wording now pulled directly from pilot_prompts.csv
#      (the file used for the manual Claude/Gemini/ChatGPT pilot) -
#      confirmed identical to what I'd used before, so nothing drifted.
#   2. Gender roles, police violence, and suicide/euthanasia wording added.
#      Verbatim wording is pulledfrom the official GSS codebook/questionnaires 
#      (gss.norc.org, thearda.com). See the `verbatim_questions` table below - 
#      each row is a single item.
#   3. Response formats now vary by item instead of being hardcoded to
#      Yes/No:
...
#      This needed a rewrite of build_prompt() and the response parser,
#      since neither could assume "1 or 2" anymore.
#   4. Years are now PER VARIABLE, not a fixed c(1982, 2002, 2022). Pulled
#      from the non-NA years in your gss_yearly_summary.csv - i.e. every
#      year that specific item was actually fielded. This is deliberately
#      NOT the same as the salience index's year range: the
#      salience_final.csv has annual coverage back to 1972, but the GSS only
#      asks most of these items in specific survey years (and not every
#      item in every survey year - abany starts in 1977, fepol in 1974,
#      etc.). Using salience years here would generate prompts for years
#      with no GSS marginal to validate against.
#   5. CONFIRMED (previously an open assumption): the binary and 4-point
...
# 507 unique variable-year combinations x 3 models x 5 runs = 7,605 base
# calls (plus a re-prompt on any refusal). At these models' prices, should come
# to well under $1 total. Runtime is several hours.
# ==============================================================================
...
RAW_OUTPUT_PATH   <- "data_raw/smoke_test_responses.csv"
CLEAN_OUTPUT_PATH <- "data_processed/smoke_test_responses_clean.csv"
COMPARISON_PATH   <- "data_processed/smoke_test_comparison.csv"
...
# ── 3 chosen models from your final 11-model list ────────────────────────────────
...
extract_clean_response <- function(text, n_options) {
  ...
  match <- str_extract(trimws(text), "^[0-9]+")
...
if (file.exists(RAW_OUTPUT_PATH)) {
  results <- read_csv(RAW_OUTPUT_PATH, show_col_types = FALSE)
...
if (file.exists(GSS_SUMMARY_PATH)) {
  ...
  simulated <- results |>
    ...
    summarise(
      simulated_value = if (first(response_type) == "binary") {
        mean(final_response == 1, na.rm = TRUE)
      } else {
        mean(final_response, na.rm = TRUE)
      },
